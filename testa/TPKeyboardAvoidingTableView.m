...
//    NSLog(@"%f",[self idealOffsetForView:firstResponder withSpace:[self keyboardRect].origin.y - self.bounds.origin.y]);
...
//    }
    ...
    NSInteger numberOfRow =[self numberOfRowsInSection:0];
    CGFloat contentSizeHight= numberOfRow * 45.0f;
//    NSLog(@"offset :%f self.contentSize.height :%f",offset , contentSizeHight);
    
    if ( contentSizeHight - offset < space ) {
        // Scroll to the bottom
        offset = contentSizeHight - space;
    } else {
        if ( view.bounds.size.height < space ) {
            // Center vertically if there's room
            offset -= floor((space-view.bounds.size.height)/2.0);
        }
        if ( offset + space > contentSizeHight ) {
            // Clamp to content size
            offset = contentSizeHight - space;
        }
    }
